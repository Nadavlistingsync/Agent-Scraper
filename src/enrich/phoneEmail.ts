...
export class PhoneEmailEnricher {
  ...
  async enrichContact(name: string, company: string, existingPhone?: string, existingEmail?: string): Promise<EnrichmentResult> {
    ...
    try {
      ...
      logError(error as Error, { name, company, context: 'PhoneEmailEnricher.enrichContact' });
      return result;
    }
  }
  
  private async enrichWithApollo(name: string, company: string): Promise<EnrichmentResult | null> {
    try {
      ...
      logError(error as Error, { name, company, context: 'PhoneEmailEnricher.enrichWithApollo' });
      return null;
    }
  }
  
  private async enrichWithHunter(company: string): Promise<EnrichmentResult | null> {
    try {
      ...
      if (response.data && response.data.data && response.data.data.emails) {
        ...
        const decisionMakerEmail = emails.find((email: any) => 
          email.position && this.isDecisionMakerTitle(email.position)
        );
      ...
      logError(error as Error, { company, context: 'PhoneEmailEnricher.enrichWithHunter' });
      return null;
    }
  }
  
  private findBestNameMatch(targetName: string, people: any[]): any | null {
    if (!targetName || !people.length) return null;
  ...
  private async getCompanyDomain(company: string): Promise<string | null> {
    try {
      ...
      logError(error as Error, { company, context: 'PhoneEmailEnricher.getCompanyDomain' });
      return null;
    }
  }
  ...
  private isDecisionMakerTitle(title: string): boolean {
    ...
    return decisionMakerTitles.some(dmTitle => 
      titleLower.includes(dmTitle.toLowerCase())
    );
  }
  ...
  async verifyPhone(phone: string): Promise<boolean> {
    try {
      ...
      logError(error as Error, { phone, context: 'PhoneEmailEnricher.verifyPhone' });
      return false;
    }
  }
  ...
  async verifyEmail(email: string): Promise<boolean> {
    try {
      ...
      logError(error as Error, { email, context: 'PhoneEmailEnricher.verifyEmail' });
      return false;
    }
  }
}
