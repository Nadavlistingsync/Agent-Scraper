...
export class BrowserSearch {
    ...
    async initialize() {
        try {
            ...
            logError(error, { context: 'BrowserSearch.initialize' });
            throw error;
        }
    }
    async searchConstructionCompanies(states = []) {
        ...
        for (const query of searchQueries.slice(0, 5)) { // Limit to 5 queries for testing
            try {
                ...
                logError(error, { query, context: 'BrowserSearch.searchConstructionCompanies' });
                continue;
            }
        }
    ...
    async performBrowserSearch(query) {
        try {
            ...
            logError(error, { query, context: 'BrowserSearch.performBrowserSearch' });
            return [];
        }
    }
...
//# sourceMappingURL=browserSearch.js.map