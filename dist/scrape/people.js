...
import { normalizePhone, normalizeEmail, isValidTitle, extractPhonesFromText, extractEmailsFromText } from '../util/normalize.js';
export class PeopleScraper {
    async extractPeopleFromPage(html, sourceUrl) {
        ...
        try {
            ...
            for (const method of extractionMethods) {
                try {
                    ...
                    logError(error, { sourceUrl, context: 'PeopleScraper.extractPeopleFromPage' });
                    continue;
                }
            }
    ...
    extractFromLeadershipPage($, sourceUrl) {
        const people = [];
    ...
    extractFromContactPage($, sourceUrl) {
        const people = [];
    ...
    extractFromAboutPage($, sourceUrl) {
        const people = [];
    ...
    extractFromGenericPage($, sourceUrl) {
        const people = [];
    ...
    extractPersonFromElement($, $element, sourceUrl) {
        try {
            const text = $element.text();
            ...
            logError(error, { sourceUrl, context: 'PeopleScraper.extractPersonFromElement' });
            return null;
        }
    }
    extractName($element, text) {
    ...
    extractPhone($element, text, html) {
    ...
    extractEmail($element, text, html) {
    ...
    extractStructuredContactData($, sourceUrl) {
        const people = [];
    ...
    extractPersonFromText(text, sourceUrl) {
        const lines = text.split('\n').map(line => line.trim()).filter(line => line);
        let name = '';
        let title = '';
        let phone = '';
