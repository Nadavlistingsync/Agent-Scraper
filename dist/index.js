    ...
    for (const result of searchResults) {
        ...
        await limit(async () => {
            try {
                ...
                logError(error, { url: result.url, context: 'processMainScraping' });
            }
        });
    }
    logProgress(`Completed main scraping: ${processedCount} URLs processed`);
}
async function processUrlWithBrowser(session, url) {
    try {
        ...
        for (const pageUrl of allPages.slice(0, 3)) { // Limit to 3 pages per company
            try {
                ...
                logError(error, { pageUrl, context: 'processUrlWithBrowser' });
                continue;
            }
        }
    }
    catch (error) {
        logError(error, { url, context: 'processUrlWithBrowser' });
    }
}
async function processUrl(session, url) {
    try {
        ...
        for (const pageUrl of allPages.slice(0, 3)) { // Limit to 3 pages per company
            try {
                ...
                logError(error, { pageUrl, context: 'processUrl' });
                continue;
            }
        }
    }
    catch (error) {
        logError(error, { url, context: 'processUrl' });
    }
}
async function processPerson(session, person, companyInfo, sourceUrl) {
    try {
        // Create lead object
        const lead = {
            Name: person.name,
        ...
        if (config.enrichment.apolloApiKey || config.enrichment.hunterApiKey) {
            const enrichment = await session.enricher.enrichContact(person.name, companyInfo.name, person.phone, person.email);
            if (enrichment.phone)
